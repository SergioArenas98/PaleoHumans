...
- A reachable Spring Boot backend on `http://127.0.0.1:8080/api` for local development (the `backend/` service in this monorepo — see [../06-local-development.md](../06-local-development.md)), or a production URL configured in `environment.prod.ts` for builds intended for deploy.
...
- **Never invent backend endpoints, fields, or filters.** The backend lives in `backend/` of this monorepo and its contract is documented in [../05-api-contract.md](../05-api-contract.md); verify against it (or the live response) and mark any unverifiable assumption explicitly.
...
| Frontend architecture | `docs/frontend/01-frontend-architecture.md` |

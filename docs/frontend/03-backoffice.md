...
| Archaeological Contexts | `/admin/archaeological-contexts` | `…/new` | `…/:id` | Site/culture/reference editing. Each row of the associated **Individuals** panel has a **Move** action ("Move to another context") that opens an accessible dialog: it loads the individual, offers a searchable/paginated destination selector (current context excluded, resolvable by id beyond the first page), confirms source → destination, and calls `POST /api/individuals/:id/move-context`. If the individual belongs to a funerary context the move is **blocked** and the dialog lists what to resolve first (mirroring the backend 409); on success it offers links to the moved individual and the destination context and reloads the source. v1 `/admin/osteological-units` redirects here. |
| Individuals | `/admin/individuals` | `…/new` | `…/:id` | Belongs to exactly one archaeological context (required FK, never detachable to an orphan state); the context can be **reassigned** to another existing context (edit form select, or the source context's Move action). |

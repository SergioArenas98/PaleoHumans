...
- **Plain white map fallback.** `/map` (`.mp-el`) and the `/sites/:id` mini-map (`.sdp-hero__map-el`) paint a plain white background (`background: #fff`) while the external basemap tiles are still on the wire; the live tiles then paint over it. There is **no** decorative or cartographic placeholder — no SVG map shell, no abstract landmasses, and no dark terrain veil. The Leaflet mount point is itself the element Leaflet upgrades to `.leaflet-container`, so the white is also asserted on the container (`app-map-page .leaflet-container { background-color: #fff !important }`, and the equivalent on `app-site-details-page`) — overriding only `background-color` (not the `background` shorthand) so cold-load shows a clean white surface and never flashes Leaflet's default grey. White matches the light OpenTopoMap physical tiles, so the transition from fallback to tiles is visually quiet. (This replaced an earlier static SVG map shell, which itself had replaced the former dark `.mp-terrain-veil` striped gradient.)
...
- **Markers no longer wait for tiles.** `MapPage.tryRenderMarkers()` renders the marker layer as soon as the Leaflet shell **and** the stats data are ready (`shellInited && dataReady`). It no longer waits for the basemap's first `load` event. Markers render over the plain white map fallback while tiles are still on the wire, so a single slow OpenTopoMap tile can no longer hold them hostage. Markers are positioned in Leaflet's coordinate space, so they align with the live tiles once those fade in. `tilesReady` now only drives the map region's `aria-busy` hint.
...
- `/map` markers re-gated behind the basemap `load` event (markers render on `shellInited && dataReady`; re-gating reintroduces the 3-9 s marker delay under slow tiles).

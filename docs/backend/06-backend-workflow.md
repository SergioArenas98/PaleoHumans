...
The authoritative PostgreSQL schema lives at
`src/main/resources/db/database.sql` (a reference copy is kept at
`docs/database/schema.sql` — see [../04-database.md](../04-database.md)).
...
- DTOs are records for create/response and mutable classes with
  ...
  [01-backend-architecture.md](./01-backend-architecture.md) and
...
2. **Do not recreate v1 entities.** Use the canonical v2 names
   listed in [../03-domain-model.md](../03-domain-model.md).
3. **Schema changes** require updating
   `src/main/resources/db/database.sql` (authoritative) **and** the
   reference copy `docs/database/schema.sql` in lockstep, then updating
   [../04-database.md](../04-database.md).
4. **API changes** require updating
   [../05-api-contract.md](../05-api-contract.md) and, if security
   boundaries change, [02-security-and-auth.md](./02-security-and-auth.md).
5. **New validation rules** belong in the service layer; record
   them in [03-validation-and-errors.md](./03-validation-and-errors.md)
   and [04-services-and-business-rules.md](./04-services-and-business-rules.md).

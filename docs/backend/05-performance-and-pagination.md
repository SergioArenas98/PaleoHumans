...
| `DatedSampleRepository.findDetailById` | `archaeologicalContext`, `archaeologicalContext.site`, `archaeologicalContext.culture`, `bone`, `bone.boneCatalog`, `bone.individuals`, `skeleton`, `skeleton.individual`, `datingResults`, `datingResults.datingTechnique` |
...
- `ArchaeologicalContextRepository.findMapTimelineUnitRows()` —
  ...
  samples). The bone count joins `bone_individual` and uses
  `COUNT(DISTINCT bone.bone_id)`, and the dated-sample count uses an
  `EXISTS` over `bone.individuals`, so neither inflates when a bone is
...
- `BoneSiteSearchRepository.findMatches(...)` — single JPQL
  constructor projection with `SIZE(b.datedSamples)`. The join through
  `bone.individuals` (bridge) yields one row per (bone, individual) pair;
  the service deduplicates by `bone_id`.
...
- The `Bone.individuals` / `Individual.bones` many-to-many is loaded
  lazily (not in the bone list/search entity graphs) to keep one row per
  bone and avoid duplicate-row / pagination hazards; both collections
  carry `@BatchSize(100)` so the response mappers batch the bridge loads
  inside the read transaction instead of issuing one query per bone.

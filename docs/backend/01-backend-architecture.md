...
4. `CacheControlInterceptor.preHandle` sets `Cache-Control` on `GET`
   ...
   [05-performance-and-pagination.md](./05-performance-and-pagination.md)).
...
See [02-security-and-auth.md](./02-security-and-auth.md) for the full
security map.
...
- treat `bone_catalog_component` as anything other than a read-only
  ...
  invalidate it (see [../04-database.md](../04-database.md));
- relax the v2 invariants enforced both in DB constraints and in
  service-layer validation (see [03-validation-and-errors.md](./03-validation-and-errors.md)

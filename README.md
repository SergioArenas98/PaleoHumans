...
The platform is split into a frontend Angular workspace and a Spring Boot backend. If your GitHub repositories are separate, run the frontend and backend commands from their respective repository roots.
...
- npm.
- A reachable backend API at `http://127.0.0.1:8080/api` for local development.

Backend:

- Java 17.
- Maven wrapper.
- PostgreSQL 13+ for dev and production.
- Environment variables for datasource and JWT configuration.
...
```bash
export SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5433/paleohumans
...
export BOOTSTRAP_ADMIN_PASSWORD=replace-with-a-secure-password
```

After creating the first admin, disable the bootstrap flag.
...
Security tooling is configured through Maven profiles and repository tooling:
...
Current backend test coverage includes context boot, auth, rate limiting, security configuration, users, sites, individuals, references, bone mapping, bone-site search, stats and dataset export. PostgreSQL-specific features such as named enums, generated columns, the `bone_catalog_component` view and DB-level CHECK constraints should be covered by future Testcontainers tests.
...
For UI work, a browser smoke test is required. Type checking and unit tests are not enough for routes involving SSR, maps, hydration or admin-auth flows.
